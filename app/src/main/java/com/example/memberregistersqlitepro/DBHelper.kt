...
class DBHelper(val context: Context?, val name: String?, val version: Int) :
    ...
    fun insertTBL(member: Member): Boolean {
        ...
        var flag = false
        try {
            db.execSQL("insert into memberTBL values( '${member.id}','${member.name}','${member.password}'," +
    ...
    fun selectLogin(id:String, password: String): Boolean {
        ...
        try {
            ...
            if(cursor.count >= 1 ){
                Log.d("DBHelper", "selectLogin 로긴성공")
                    flag = true
            }
        }catch (e: SQLException){
            Log.d("DBHelper", "selectLogin ${id} 로긴예외발생 ${e.printStackTrace()}")
            flag = false
        }
        return flag
    }
    ...
    fun selectID(id: String): Member?{
        ...
        try {
            ...
            if(cursor.moveToFirst()){
                Log.d("DBHelper", "selectID ${id} 데이타정보 로드성공")
                member = Member(cursor.getString(0), cursor.getString(1), cursor.getString(2),
        ...
        try {
            ...
            if(cursor.count >= 1) {
                ...
                while(cursor.moveToNext()){
                    val member = Member(cursor.getString(0), cursor.getString(1),
    ...
    fun deleteTBL(id: String): Boolean{
        ...
        try {
            if (selectCheckID(id)){
                ...
                Log.d("DBHelper", "delete ${id}  존재하지않음")
                flag = false
            }
        }catch (e: SQLException){
    ...
    fun updateTBL(member: Member?): Boolean{
        ...
        try {
            if(member != null){
                db.execSQL("update memberTBL set phone = '${member?.phone}',name = '${member?.name}',level = '${member?.level}'" +
                        " where id = '${member?.id}'")
                Log.d("DBHelper", "update ${member?.id}  성공")
                flag = true
            }
        }catch (e: SQLException){
                Log.d("DBHelper", "update  실패예외발생 ${e.printStackTrace()}")
        }
        return flag
    }
}
